...
import {controlKeyboard, isSplitCallback, parseSplitCallback, splitKeyboard} from "@/ui/workout.keyboards";
...
async function safeEditCard(ctx: Context, chatId: number, session: WorkoutSession, html: string): Promise<void> {
...
export async function registerCommands(bot: Bot<Context>): Promise<void> {
    ...
    bot.command("w", async (ctx) => {
        ...
        if (arg && !date) {
            await ctx.reply("<b>Неверная дата</b>\n<i>Формат: /w 04.12.2025</i>", {parse_mode: "HTML"});
            return;
        }
    ...
    bot.command("date", async (ctx) => {
        ...
        if (!date) {
            await ctx.reply("<b>Неверная дата</b>\n<i>Формат: /date 04.12.2025</i>", {parse_mode: "HTML"});
            return;
        }
    ...
    bot.command("done", async (ctx) => {
        ...
        if (Number.isFinite(managerChatId)) {
            try {
                await ctx.api.sendMessage(managerChatId as number, message, {parse_mode: "HTML"});
            } catch {
                // no-op
            }
        }
    ...
    bot.command("cancel", async (ctx) => {
        ...
        await ctx.reply(had ? "<b>Тренировка отменена</b>" : "<b>Нет активной тренировки</b>", {parse_mode: "HTML"});
    });
    ...
    bot.callbackQuery(/.*/, async (ctx) => {
        ...
        if (data === "workout:done") {
            const workout = service.finalize(chatId);
            if (!workout) {
                await ctx.answerCallbackQuery({text: "Нечего завершать", show_alert: false});
                return;
            }
        ...
        if (data === "workout:cancel") {
            service.cancel(chatId);
            await ctx.editMessageText("<b>Тренировка отменена</b>", {parse_mode: "HTML"});
        ...
        if (isSplitCallback(data)) {
            ...
            if (!session) {
                await ctx.answerCallbackQuery({text: "Нет активной тренировки", show_alert: false});
                return;
            }
